...
verify_csob_inputs <-
  function(dir_name,
    ...
    stopifnot(
      ...
      is.character(temporary_cookie_csob) &&
        nchar(temporary_cookie_csob) >= 10,
...
get_csob_transactions <-
  function(bank_accounts,
    ...
    print(paste(
      length(bank_accounts),
      "bank account(s) selected, will run the function."
    ))
  ...
  \"paging\": {
    ...
    \"pageNumber\": 1
      ...
      nr_new_records <-
        transactions_list[[names(bank_accounts)[i]]][["paging"]][["recordCount"]]

      # Skip to next loop iteration if there are no returned transactions
      if (nr_new_records < 1) {
        print(
          paste(
            "No transactions on the account of entity",
            names(bank_accounts)[i],
            "between",
      ...
      print(
        paste(
          nrow(transactions_list[[names(bank_accounts)[i]]]),
          "transactions on the account of entity",
          names(bank_accounts)[i],
          "between",
          format(as.Date(start_date), "%d.%m.%Y"),
          "and",
