...
scrape_fio_accounts <- function(accounts_fio, dir_name, start_date, end_date) {
  
  if (!is.null(accounts_fio$numbers)) {
    print(paste(length(accounts_fio$numbers), "bank account(s) selected, will attempt to run the function."))
    ...
    for (i in seq_along(accounts_fio[["numbers"]])) {
      ...
      fio_tables <- read_html(paste0("https://ib.fio.cz/ib/transparent?a=", accounts_fio[["numbers"]][i], "&f=", start_date, "&t=", end_date)) %>% 
        html_table(header = TRUE, dec = ",")
      
      
      if (length(fio_tables) == 1) {
        print(paste("No transactions on the account of entity", accounts_fio[["names"]][i], "between", start_date, "and", end_date))
    ...
    } else if (dim(transactions_df)[1] == 0) {
      print(paste("No transactions on any of the selected bank accounts between", start_date, "and", end_date, "no need to append"))
    }
    
  } else if (is.null(accounts_fio$numbers)) {
    print("No bank accounts selected, skipping the extraction.")
  }
}
...
scrape_fio_accounts(accounts_fio = readRDS("data/list_of_monitored_accounts.rds")[["fio"]],
                    dir_name = "data",
